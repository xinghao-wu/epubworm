...
auto dispatchCli(int argc, char** argv) -> int {
  ...
  case CliCommand::none: {
    ...
    if (library.LoadFile(libraryFileAbs.c_str()) != XML_SUCCESS) {
      throw std::runtime_error{std::string{"error loading library file: "}
  ...
  case CliCommand::add: {
    ...
    for (const auto& result : results) {
      if (!result.has_value()) {
        ...
        printLibraryUpdate("Added: ", greenFG, result.value(), libraryRoot);
      }
      ++argIndex;
    }
    if (addError != nullptr) {
  ...
  case CliCommand::setLineLength: {
    ...
    const auto [end, error]{
        std::from_chars(value.data(), value.data() + value.size(), chars)};
