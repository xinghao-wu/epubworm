...
// OPF at `shareAbs/epubworm/extracted_epubs/<id>/`. Prints the last-read
// epub's title after the entries, or a message if it is no longer in the
// library. Prints a message indicating the library is empty if there are no
// `<epub>` entries. Status messages are styled when stdout is a terminal.
